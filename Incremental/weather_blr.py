...
def fetch_weather_asset(context):
    ...
    return fetch_weather_blr_data(api_key, location, dt)



#Extracting and transforming the data 
def fetch_weather_blr_data(api_key: str, location: str, dt: str) -> dict:
...
def load_data_to_snowflake(df: pd.DataFrame):
    ...
    df.to_sql('weather_bangalore', con=engine, if_exists="append", index=False)
...
@schedule(cron_schedule="0 * * * *", job=weather_blr_job, execution_timezone="UTC")
def hourly_weather_blr_schedule(_context):
    return {
        'ops': {
            'fetch_weather_asset': {
                'config': {
...
def main():
    ...
    result = weather_blr_job.execute_in_process(run_config=config)
